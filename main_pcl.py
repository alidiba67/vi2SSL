...
import pcl.builder
import pcl.augmentation as A
from pcl.dataset import VideoDataset

model2D_names = sorted(name for name in models.__dict__
...
parser.add_argument('-a2D', '--arch2D', metavar='ARCH2D', default='resnet50',
                    choices=model2D_names,
                    help='2D model architecture: ' +
                        ' | '.join(model2D_names) +
                        ' (default: resnet50)')
parser.add_argument('-a3D', '--arch3D', metavar='ARCH3D', default='s3d',
                    help='3D model architecture: s3d, s3dg')
...
parser.add_argument('--no_check', action='store_true',
                    help='do not check length of videos')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    print("=> creating models 2D: {},  3D: {}".format(args.arch2D, args.arch3D))
    ...
    transform_eval = get_transform('eval', args)

    train_dataset = get_data(transform_train, 'train', args)
    eval_dataset = get_data(transform_eval, 'eval', args)
    ...
    # use larger batch size to increase speed
...
def train(train_loader, model, criterion, optimizer, epoch, args, cluster_result=None):
    ...
    for i, (clips, images, index) in enumerate(train_loader):
        ...
        image_k = images[:, 1]
        if args.gpu is not None:
            clip_q = clip_q.cuda(args.gpu, non_blocking=True)
            clip_k = clip_k.cuda(args.gpu, non_blocking=True)
            image_q = image_q.cuda(args.gpu, non_blocking=True)
            image_k = image_k.cuda(args.gpu, non_blocking=True)

        # compute output
        output, target, output_proto, target_proto = model(in_q=[clip_q, image_q], in_k=[clip_k, image_k],
        ...
        if output_proto is not None:
            ...
            for proto_out, proto_target in zip(output_proto, target_proto):
                ...
                acc_proto.update(accp[0], images.size(0))
        ...
        acc_inst.update(acc[0], images.size(0))
...
def compute_features(eval_loader, model, args):
    ...
    for i, (clips, images, index) in enumerate(tqdm(eval_loader)):
        with torch.no_grad():
            clips = clips[:, 0].cuda(non_blocking=True)
            images = images[:, 0].cuda(non_blocking=True)
            feat = model([clips, images], is_eval=True)
            features[index] = feat
    dist.barrier()        
...
def get_transform(mode, args):
    ...
    if mode == 'train':
        ...
        transform = transforms.Compose([
            A.Scale(size=int(args.img_dim * 1.143)),
...
def get_data(transform, mode, args):
    ...
    dataset = VideoDataset(root=args.data,
                           mode='train', transform=transform,
                           seq_len=args.seq_len, num_seq=(1, 2)[mode == 'train'],
                           downsample=args.downsample, checklength=not args.no_check)
